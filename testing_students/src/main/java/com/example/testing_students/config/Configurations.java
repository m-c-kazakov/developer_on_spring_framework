...
import com.example.testing_students.UserInterface;
...
public class Configurations {
    ...
    @Bean
    public UserInterface userInterface(@Value("${fileName}") String fileName,
                                       @Value("${language}") String tag,
                                       MessageSender messageSender,
                                       MessageSource messageSource,
                                       FileReader fileReader) {
        Map<String, String> questionAndAnswerMap = fileReader.read(fileName+"_"+tag);
        return new UserInterface(messageSender, messageSource, LocaleUtils.toLocale(tag) , questionAndAnswerMap);
    }
}
